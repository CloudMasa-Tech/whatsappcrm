...
The public API lets you drive your MaSa CRM instance from your own
...
```
Authorization: Bearer masacrm_live_xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx
```
...
3. Copy the key. **The full key is shown exactly once.** MaSa CRM
...
curl https://your-crm.example.com/api/v1/me \
  -H "Authorization: Bearer masacrm_live_xxx"
```
...
Rather than polling, register an endpoint and MaSa CRM will POST to it when
...
- `POST /api/v1/webhooks` — register `{ "url": "https://…", "events": ["message.received"] }`. `url` must be `https://`. **The response includes `secret` exactly once** — store it to verify signatures; MaSa CRM keeps only an encrypted copy.
...
curl -X POST https://your-crm.example.com/api/v1/webhooks \
  -H "Authorization: Bearer masacrm_live_xxx" \
  -H "Content-Type: application/json" \
  -d '{ "url": "https://example.com/hooks/MaSa CRM", "events": ["message.received"] }'
...
Headers: `X-MasaCRM-Event`, `X-MasaCRM-Webhook-Id`, and `X-MasaCRM-Signature`.

### Verifying the signature

`X-MasaCRM-Signature: t=<unix_seconds>,v1=<hex>` where `v1 =
...
covers messages MaSa CRM stores (inbox + API sends), not broadcast-only
...
[#245](https://github.com/ArnasDon/MaSa CRM/issues/245). Future ideas
