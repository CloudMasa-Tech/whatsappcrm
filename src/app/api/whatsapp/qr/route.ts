...
export async function GET(request: Request) {
  try {
    ...
    if (ctx.project.channel_type !== 'qr') {
      return NextResponse.json({
        channel_type: ctx.project.channel_type,
        session: null,
        message:
          'This project uses the Cloud API channel. QR pairing applies only to QR projects.',
      })
    }
  ...
  try {
    ...
    if (ctx.project.channel_type !== 'qr') {
      return NextResponse.json(
        {
          error:
            'This project uses the Cloud API channel. Create a QR project to pair a number by scanning.',
          code: 'wrong_channel',
        },
        { status: 409 },
      )
