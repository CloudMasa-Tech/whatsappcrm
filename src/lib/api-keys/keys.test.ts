...
describe('hashApiKey', () => {
  it('is deterministic', () => {
    expect(hashApiKey('masacrm_live_abc')).toBe(hashApiKey('masacrm_live_abc'));
  });

  it('differs for different inputs', () => {
    expect(hashApiKey('masacrm_live_abc')).not.toBe(hashApiKey('masacrm_live_abd'));
  });
});
...
describe('timingSafeHexEqual', () => {
  it('is true for identical digests', () => {
    const h = hashApiKey('masacrm_live_xyz');
