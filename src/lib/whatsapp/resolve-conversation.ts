...
export async function resolveConversationByPhone(
  ...
  // connected — the same error the send would raise anyway. Check both
  // Cloud API (whatsapp_config) and QR (whatsapp_sessions) since both
  ...
  const cloudConnected = !!configResult.data;
  const qrConnected = sessionResult.data?.status === 'connected';

  if (!cloudConnected && !qrConnected) {
    throw new SendMessageError(
      'whatsapp_not_configured',
      'WhatsApp is not connected for this project. Connect via QR code or Cloud API in Settings.',
      400
    );
  }
