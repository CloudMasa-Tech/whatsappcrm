...
import { hasMinRole, isAccountRole, isPlatformRole, type AccountRole, type PlatformRole } from "./roles";
...
export interface AccountContext {
  ...
  role: AccountRole;
  /** Caller's platform role — super_admin or customer. */
...
export async function getCurrentAccount(): Promise<AccountContext> {
  ...
  // Try with platform_role first (migration 047). If the column
  // doesn't exist yet, fall back to a query without it.
  const primaryCols = "account_id, account_role, platform_role";
  const fallbackCols = "account_id, account_role";
  ...
  let data: { account_id: string | null; account_role: string | null; platform_role?: string | null } | null = null;
  let error: typeof primary.error = null;

  if (primary.error && primary.error.code === "42703") {
    // platform_role column does not exist — fall back
  ...
  // with profiles that pre-date migration 047.
...
/**
 * Require the caller to be a platform super_admin.
 *
 * Throws `ForbiddenError` if the caller's platform_role is not
 * `super_admin`. Used by admin-only API routes.
 */
