...
export async function proxy(request: NextRequest) {
  ...
  if (user) {
    try {
      ...
      // Customers must not access CRM-only routes (hidden from sidebar, blocked here too)
      if (platformRole === 'customer') {
        const crmOnlyPaths = ['/pipelines', '/automations', '/flows', '/agents', '/inbox', '/notifications']
        if (crmOnlyPaths.some(path => request.nextUrl.pathname === path || request.nextUrl.pathname.startsWith(path + '/'))) {
          const url = request.nextUrl.clone()
          url.pathname = '/dashboard'
          return withRefreshedCookies(NextResponse.redirect(url))
        }
