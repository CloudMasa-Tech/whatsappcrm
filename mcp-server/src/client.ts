...
/** A structured error from the masacrm API envelope (`{ error: { code, message } }`). */
  ...
  private async request<T>(
    ...
    try {
      ...
      throw new WacrmApiError(
        0,
        'network_error',
        `Could not reach masacrm at ${this.baseUrl}: ${(err as Error).message}`,
      );
    }
